...
const LoadingProvider = ({ children }) => {
  ...
  const getRecipes = () => {
    ...
    if (!recipes) {
      ...
      axios.get('https://api.spoonacular.com/food/search?apiKey=224dd7450e7d4cfa9bf65a747f11fd61&number=20')
      .then((response) => {
  ...
  const getRecipeDetails = (id) => {
    ...
    setIsLoading(true);
    axios
      .get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=224dd7450e7d4cfa9bf65a747f11fd61`)
      .then((response) => {
  ...
  return(
      <LoadingContext.Provider value={{ deleteRecipe, setUser, user, setMessage, setTimedMessage, setIsLoading, isLoading, getRecipes, recipes, getRecipeDetails, getUserRecipeDetails, recipeDetails, getUserRecipes, userRecipes, setUserRecipeDetails, userRecipeDetails }}>
        {children}
